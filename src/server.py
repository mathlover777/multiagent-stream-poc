...
from agents import weather_agent, holiday_agent, triage_agent
...
chat_streamer = ChatStreamMongoMemory(
...
async def chat_complete_multi_agent(request: Request, chat_id: str = Query(...), message: str = Query(...)):
    ...
    async def event_generator():
        async for event in chat_streamer.stream_chat(chat_id, message):
